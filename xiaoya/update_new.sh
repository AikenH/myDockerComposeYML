...
if [ $1 == 'host' ]; then
	...
	docker pull registry.hub.docker.com/xiaoyaliu/alist:hostmode
...
docker stop xiaoya
docker rm xiaoya
docker pull registry.hub.docker.com/xiaoyaliu/alist:latest
docker run -d -p 15678:80 -v /etc/xiaoya:/data --restart=always --name=xiaoya xiaoyaliu/alist:latest
